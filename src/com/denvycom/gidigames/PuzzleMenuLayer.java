...
public class PuzzleMenuLayer extends CCColorLayer implements CCTableViewDelegate, CCTableViewDataSource{
	...
	{ 
		GidiGamesActivity.nextscene= "MenuLayer" ;
		 GidiGamesActivity.currentscene = "PuzzleMenuLayer" ;
		CCScene scene = CCScene.node();
		CCLayer layer = new PuzzleMenuLayer();
	...
	protected PuzzleMenuLayer()
	{
		...
		// Add Title
		CCBitmapFontAtlas selectgame = CCBitmapFontAtlas.bitmapFontAtlas ("PUZZLEMANIA - SELECT FLAVOUR", "dalek.fnt");
		...
		array.add("Number Puzzle"); 
		array.add("Letter Puzzle"); 
		array.add("Picture Puzzle");
	...
	public void tableCellTouched(CCTableView table, CCTableViewCell cell)
	{
		...
		if (indexselected == 0){
			...
			CCScene next = CCSlideInLTransition.transition(0.2f, PuzzleNumLayer.scene("number"));
			CCDirector.sharedDirector().replaceScene( next );
		}else if(indexselected == 1){
			GidiGamesActivity.clicksound() ;
			CCScene next = CCSlideInLTransition.transition(0.2f, PuzzleNumLayer.scene("letter"));
			CCDirector.sharedDirector().replaceScene( next );
		}else if(indexselected == 2){
			GidiGamesActivity.clicksound() ;
			CCScene next = CCSlideInLTransition.transition(0.2f, PuzzlePicLayer.scene("picture"));
			CCDirector.sharedDirector().replaceScene( next );
		}
	}
