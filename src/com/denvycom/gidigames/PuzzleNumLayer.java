...
public class PuzzleNumLayer extends CCColorLayer implements CCTableViewDelegate, CCTableViewDataSource{
	...
	{ 
		GidiGamesActivity.nextscene= "PuzzleMenuLayer" ;
		 GidiGamesActivity.currentscene = "PuzzleNumLayer" ;
		 GidiGamesActivity.currentpuzzletype = puzzletype ;
		PUZZLE_TYPE =  puzzletype ;
		CCScene scene = CCScene.node();
		CCLayer layer = new PuzzleNumLayer();
	...
	protected PuzzleNumLayer()
	{
		...
		if (PUZZLE_TYPE.equals("picture")){
			toptext =  GidiGamesActivity.currentpictitle+ " PICTURE PUZZLE";
		}else {
			
			toptext = "PUZZLEMANIA - " + PUZZLE_TYPE + "S" ;
		}

		// Add Title
		CCBitmapFontAtlas selectgame = CCBitmapFontAtlas.bitmapFontAtlas (toptext, "dalek.fnt");
	...
	public void tableCellTouched(CCTableView table, CCTableViewCell cell)
	{
		...
		if (indexselected == 0){
			CCScene next = CCSlideInLTransition.transition(0.2f, PuzzleLayer.scene(3,3,PUZZLE_TYPE));
			CCDirector.sharedDirector().replaceScene( next );
		}else if(indexselected == 1){
			CCScene next = CCSlideInLTransition.transition(0.2f, PuzzleLayer.scene(4,4,PUZZLE_TYPE));
			CCDirector.sharedDirector().replaceScene( next );
		}else if(indexselected == 2){
			CCScene next = CCSlideInLTransition.transition(0.2f, PuzzleLayer.scene(5,5,PUZZLE_TYPE));
			CCDirector.sharedDirector().replaceScene( next );
		}
	}
	...
	public CCTableViewCell tableCellAtIndex(CCTableView table, int idx)
	{
		...
		CCSprite image = CCSprite.sprite("puzzlebox.png");
