...
//        m_indices           = new CCMutableArray<int*>();
    ...
    public CCTableViewCell dequeueBitmapCell()
    {
        CCTableViewBitMapFontCell cell;
        
        if (m_cellsFreed.size() == 0) {
            return null;
        } else {
            cell = (CCTableViewBitMapFontCell)m_cellsFreed.get(0);
            m_cellsFreed.remove(0);
        }
        return cell;
    }
	...
	public boolean ccTouchesEnded(MotionEvent event) {
      ...
      //  if (touches_.size() == 1 && !touchMoved_) {
        if (!touchMoved_) {
        	ccMacros.CCLOG("Ended"," --- Tapped through  "  );
            int		          index;
            CCTableViewCell   cell;
            CGPoint           point;
