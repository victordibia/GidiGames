...
//@property (nonatomic, assign) CGFloat maxZoomScale;
	...
	{
        viewSize   = size;
        tapsensitivity = (CCDirector.sharedDirector().winSize().height /480 ) * 10;
...
//    }
	...
	public boolean ccTouchesBegan(MotionEvent event) {
        ...
        locationbegan = CCDirector.sharedDirector().convertToGL(CGPoint.ccp(event.getX(), event.getY()));
...
//        } 
	...
	public boolean ccTouchesMoved(MotionEvent event) {
	    ...
	    CGPoint location = CCDirector.sharedDirector().convertToGL(CGPoint.ccp(event.getX(), event.getY()));
		 if (location.x == locationbegan.x && location.y ==locationbegan.y || ( Math.abs(location.x - locationbegan.x) < tapsensitivity && Math.abs(location.y - locationbegan.y) < tapsensitivity)  ){
			 touchMoved_=false ;
			 ccMacros.CCLOG("Ended"," --- ITs just a tap "  );
		 } else {
        touchMoved_  = true;
		 }
//	    if (touches_.contains(event)) {
